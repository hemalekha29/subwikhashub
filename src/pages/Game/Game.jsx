...
const gameIndex =
  Math.floor(Date.now() / (7 * 24 * 60 * 60 * 1000)) % GAMES.length;
...
function updateStreak() {
  let prev = null;
  try {
    prev = JSON.parse(localStorage.getItem("subwikha_streak") || "null");
  } catch {
    /* ignore */
  }
  ...
  const next = {
    ...
    bestStreak: Math.max(count, prev?.bestStreak || 0),
  };
...
function getPlayerName() {
  ...
  if (!name) {
    name = (
      window.prompt(
        "Pick a display name for the weekly leaderboard (optional):",
        "",
      ) || "Anonymous"
    ).slice(0, 24);
...
async function recordLeaderboardEntry(gameTitle, percent) {
  try {
    ...
    /* best-effort, never blocks the discount flow */
  }
}
  ...
  return (
    <div className={`page-container ${styles.page}`}>
      <Helmet>
        ...
        <meta
          name="description"
          content="Play fun mini-games at Subwikha's Hub and win up to 10% OFF on your next order. Memory match, word scramble, gift quiz and more!"
        />
        <meta
          name="keywords"
          content="gift discount game, play and win, Subwikha's Hub game zone"
        />
        <link rel="canonical" href="https://subwikhahub.vercel.app/game" />
        <meta
          property="og:title"
          content="Play & Win Discounts | Subwikha's Hub"
        />
        <meta
          property="og:description"
          content="Win up to 10% OFF by playing mini-games — Memory Match, Word Scramble, Gift Quiz and more!"
        />
        <meta property="og:url" content="https://subwikhahub.vercel.app/game" />
        <meta
          property="og:image"
          content="https://subwikhahub.vercel.app/logo.png"
        />
      </Helmet>
      ...
      <div className={styles.header}>
        {activeSeason && (
          <span
            style={{
              ...
              borderRadius: 20,
              fontSize: "0.75rem",
              border: `1px solid ${activeSeason.accent}`,
              color: activeSeason.accent,
            }}
          >
            {activeSeason.label}
          </span>
        ...
        <p className={styles.subtitle}>
          Play the weekly game to unlock a discount on your order. Fastest &amp;
          smartest wins 10% off!
        </p>
        {streak?.count > 1 && (
          <p
            style={{ color: "var(--gold)", fontSize: "0.85rem", marginTop: 4 }}
          >
            🔥 {streak.count}-day streak — playing today keeps your bonus
            growing (best: {streak.bestStreak})
          </p>
        )}
      ...
      {result === null ? (
        ...
        <div className={styles.result}>
          ...
          <h2 className={styles.resultTitle}>
            {result.restored
              ? `You already won ${result.percent}% off!`
